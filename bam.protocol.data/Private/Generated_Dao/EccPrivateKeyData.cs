...
namespace Bam.Protocol.Data.Private.Dao
{
	...
	public partial class EccPrivateKeyData: Bam.Data.Dao
	{
		...
		public EccPrivateKeyData(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public EccPrivateKeyData(IDatabase db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public EccPrivateKeyData(IDatabase db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		[Bam.Exclude]
		public static implicit operator EccPrivateKeyData(DataRow data)
		{
			return new EccPrivateKeyData(data);
		}
    ...
    public DateTime? Created
    {
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new EccPrivateKeyDataColumns();
				return (colFilter.KeyColumn == GetDbId());
			}
		}

		/// <summary>
        /// Return every record in the EccPrivateKeyData table.
        /// </summary>
		/// <param name="database">
		...
		public static EccPrivateKeyDataCollection LoadAll(IDatabase database = null!)
		{
            ...
            sql.Select<EccPrivateKeyData>();
            var results = new EccPrivateKeyDataCollection(db, sql.ExecuteGetDataTable(db))
            {
                Database = db
            };
            return results;
        ...
        public static async Task BatchAll(int batchSize, Action<IEnumerable<EccPrivateKeyData>> batchProcessor, IDatabase database = null!)
		{
			...
			{
				EccPrivateKeyDataColumns columns = new EccPrivateKeyDataColumns();
				var orderBy = Bam.Data.Order.By<EccPrivateKeyDataColumns>(c => c.KeyColumn, Bam.Data.SortOrder.Ascending);
		...
		/// </summary>
		/// <param name="where">A Func delegate that recieves a EccPrivateKeyDataColumns
		...
		/// Execute a query and return the results.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a EccPrivateKeyDataColumns
		...
		/// WhereDelegate`EccPrivateKeyDataColumns`.
		/// </summary>
		...
		public static EccPrivateKeyDataCollection Top(int count, WhereDelegate<EccPrivateKeyDataColumns> where, OrderBy<EccPrivateKeyDataColumns> orderBy, IDatabase database = null!)
		{
			...
			query.Top<EccPrivateKeyData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<EccPrivateKeyDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<EccPrivateKeyDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		private static EccPrivateKeyData OneOrThrow(EccPrivateKeyDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
