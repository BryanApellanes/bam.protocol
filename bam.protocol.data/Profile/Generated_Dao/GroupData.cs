...
namespace Bam.Protocol.Data.Profile.Dao
{
	...
	[Serializable]
	[Bam.Data.Table("GroupData", "ProfileSchema")]
	public partial class GroupData: Bam.Data.Dao
	{
		...
		public GroupData(IDatabase db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		[Bam.Exclude]
		public static implicit operator GroupData(DataRow data)
		{
		...
		private void SetChildren()
		{
			...
			if(_database != null)
			{
				this.ChildCollections.Add("GroupDataPersonData_GroupDataId", new GroupDataPersonDataCollection(Database.GetQuery<GroupDataPersonDataColumns, GroupDataPersonData>((c) => c.GroupDataId == GetULongValue("Id", false)), this, "GroupDataId"));
			}
            this.ChildCollections.Add("GroupData_GroupDataPersonData_PersonData",  new XrefDaoCollection<GroupDataPersonData, PersonData>(this, false));
	...
	[Bam.Exclude]
	public GroupDataPersonDataCollection GroupDataPersonDatasByGroupDataId
	{
		get
		{
			if (this.IsNew)
			...
			if(!this.ChildCollections.ContainsKey("GroupDataPersonData_GroupDataId"))
			{
				SetChildren();
			}

			var c = (GroupDataPersonDataCollection)this.ChildCollections["GroupDataPersonData_GroupDataId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
		...
		// Xref       
        public XrefDaoCollection<GroupDataPersonData, PersonData> PersonDatas
        {
            get
            {			
				if (this.IsNew)
				...
				if(!this.ChildCollections.ContainsKey("GroupData_GroupDataPersonData_PersonData"))
				{
					SetChildren();
				}

				var xref = (XrefDaoCollection<GroupDataPersonData, PersonData>)this.ChildCollections["GroupData_GroupDataPersonData_PersonData"];
				if(!xref.Loaded)
				{
					xref.Load(Database);
				}
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new GroupDataColumns();
				return (colFilter.KeyColumn == GetDbId());
			}
		}

		/// <summary>
        /// Return every record in the GroupData table.
        /// </summary>
		/// <param name="database">
		...
		public static GroupDataCollection LoadAll(IDatabase database = null!)
		{
            ...
            sql.Select<GroupData>();
            var results = new GroupDataCollection(db, sql.ExecuteGetDataTable(db))
            {
                Database = db
            };
            return results;
        ...
        public static async Task BatchAll(int batchSize, Action<IEnumerable<GroupData>> batchProcessor, IDatabase database = null!)
		{
			...
			{
				GroupDataColumns columns = new GroupDataColumns();
				var orderBy = Bam.Data.Order.By<GroupDataColumns>(c => c.KeyColumn, Bam.Data.SortOrder.Ascending);
		...
		/// </summary>
		/// <param name="where">A Func delegate that recieves a GroupDataColumns
		...
		/// Execute a query and return the results.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a GroupDataColumns
		...
		/// WhereDelegate`GroupDataColumns`.
		/// </summary>
		...
		public static GroupDataCollection Top(int count, WhereDelegate<GroupDataColumns> where, OrderBy<GroupDataColumns> orderBy, IDatabase database = null!)
		{
			...
			query.Top<GroupData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<GroupDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<GroupDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		private static GroupData OneOrThrow(GroupDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
