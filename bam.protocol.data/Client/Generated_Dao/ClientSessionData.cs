...
namespace Bam.Protocol.Data.Client.Dao
{
	...
	[Serializable]
	[Bam.Data.Table("ClientSessionData", "ClientSessionSchema")]
	public partial class ClientSessionData: Bam.Data.Dao
	{
		public ClientSessionData():base()
		{
			this.SetKeyColumnName();
			this.SetChildren();
		}

		public ClientSessionData(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public ClientSessionData(IDatabase db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public ClientSessionData(IDatabase db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		[Bam.Exclude]
		public static implicit operator ClientSessionData(DataRow data)
		{
			return new ClientSessionData(data);
		}
		...
		private void SetChildren()
		{
			...
			if(_database != null)
			{
				this.ChildCollections.Add("ClientSessionKeyValue_ClientSessionDataId", new ClientSessionKeyValueCollection(Database.GetQuery<ClientSessionKeyValueColumns, ClientSessionKeyValue>((c) => c.ClientSessionDataId == GetULongValue("Id", false)), this, "ClientSessionDataId"));
			}
	...
	[Bam.Exclude]
	public ClientSessionKeyValueCollection ClientSessionKeyValuesByClientSessionDataId
	{
		get
		{
			if (this.IsNew)
			...
			if(!this.ChildCollections.ContainsKey("ClientSessionKeyValue_ClientSessionDataId"))
			{
				SetChildren();
			}

			var c = (ClientSessionKeyValueCollection)this.ChildCollections["ClientSessionKeyValue_ClientSessionDataId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new ClientSessionDataColumns();
				return (colFilter.KeyColumn == GetDbId());
			}
		}

		/// <summary>
        /// Return every record in the ClientSessionData table.
        /// </summary>
		/// <param name="database">
		...
		public static ClientSessionDataCollection LoadAll(IDatabase database = null!)
		{
            ...
            sql.Select<ClientSessionData>();
            var results = new ClientSessionDataCollection(db, sql.ExecuteGetDataTable(db))
            {
                Database = db
            };
            return results;
        ...
        public static async Task BatchAll(int batchSize, Action<IEnumerable<ClientSessionData>> batchProcessor, IDatabase database = null!)
		{
			...
			{
				ClientSessionDataColumns columns = new ClientSessionDataColumns();
				var orderBy = Bam.Data.Order.By<ClientSessionDataColumns>(c => c.KeyColumn, Bam.Data.SortOrder.Ascending);
		...
		/// </summary>
		/// <param name="where">A Func delegate that recieves a ClientSessionDataColumns
		...
		/// Execute a query and return the results.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a ClientSessionDataColumns
		...
		/// WhereDelegate`ClientSessionDataColumns`.
		/// </summary>
		...
		public static ClientSessionDataCollection Top(int count, WhereDelegate<ClientSessionDataColumns> where, OrderBy<ClientSessionDataColumns> orderBy, IDatabase database = null!)
		{
			...
			query.Top<ClientSessionData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<ClientSessionDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<ClientSessionDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		private static ClientSessionData OneOrThrow(ClientSessionDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
