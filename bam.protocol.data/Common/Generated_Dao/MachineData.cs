...
namespace Bam.Protocol.Data.Common.Dao
{
	...
	[Serializable]
	[Bam.Data.Table("MachineData", "CommonSchema")]
	public partial class MachineData: Bam.Data.Dao
	{
		...
		public MachineData(DataRow data)
			: base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public MachineData(IDatabase db)
			: base(db)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		public MachineData(IDatabase db, DataRow data)
			: base(db, data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		...
		[Bam.Exclude]
		public static implicit operator MachineData(DataRow data)
		{
			return new MachineData(data);
		}
		...
		private void SetChildren()
		{
			...
			if(_database != null)
			{
				this.ChildCollections.Add("HostAddressData_MachineDataId", new HostAddressDataCollection(Database.GetQuery<HostAddressDataColumns, HostAddressData>((c) => c.MachineDataId == GetULongValue("Id", false)), this, "MachineDataId"));
			}
			if(_database != null)
			{
				this.ChildCollections.Add("NicData_MachineDataId", new NicDataCollection(Database.GetQuery<NicDataColumns, NicData>((c) => c.MachineDataId == GetULongValue("Id", false)), this, "MachineDataId"));
			}
	...
	[Bam.Exclude]
	public HostAddressDataCollection HostAddressDatasByMachineDataId
	{
		get
		{
			if (this.IsNew)
			...
			if(!this.ChildCollections.ContainsKey("HostAddressData_MachineDataId"))
			{
				SetChildren();
			}

			var c = (HostAddressDataCollection)this.ChildCollections["HostAddressData_MachineDataId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
	...
	[Bam.Exclude]
	public NicDataCollection NicDatasByMachineDataId
	{
		get
		{
			if (this.IsNew)
			...
			if(!this.ChildCollections.ContainsKey("NicData_MachineDataId"))
			{
				SetChildren();
			}

			var c = (NicDataCollection)this.ChildCollections["NicData_MachineDataId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
		...
		public override IQueryFilter GetUniqueFilter()
		{
			...
			{
				var colFilter = new MachineDataColumns();
				return (colFilter.KeyColumn == GetDbId());
			}
		}

		/// <summary>
        /// Return every record in the MachineData table.
        /// </summary>
		/// <param name="database">
		...
		public static MachineDataCollection LoadAll(IDatabase database = null!)
		{
            ...
            sql.Select<MachineData>();
            var results = new MachineDataCollection(db, sql.ExecuteGetDataTable(db))
            {
                Database = db
            };
            return results;
        ...
        public static async Task BatchAll(int batchSize, Action<IEnumerable<MachineData>> batchProcessor, IDatabase database = null!)
		{
			...
			{
				MachineDataColumns columns = new MachineDataColumns();
				var orderBy = Bam.Data.Order.By<MachineDataColumns>(c => c.KeyColumn, Bam.Data.SortOrder.Ascending);
		...
		/// </summary>
		/// <param name="where">A Func delegate that recieves a MachineDataColumns
		...
		/// Execute a query and return the results.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a MachineDataColumns
		...
		/// WhereDelegate`MachineDataColumns`.
		/// </summary>
		...
		public static MachineDataCollection Top(int count, WhereDelegate<MachineDataColumns> where, OrderBy<MachineDataColumns> orderBy, IDatabase database = null!)
		{
			...
			query.Top<MachineData>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<MachineDataColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<MachineDataCollection>(0);
			results.Database = db;
			return results;
		}
		...
		private static MachineData OneOrThrow(MachineDataCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
