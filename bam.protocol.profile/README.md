...
| `KeySetRevocation` | Implements `IKeySetRevocation`: break-glass revocation that verifies an admin proof, tombstones the key set (`RevokedUtc`/`RevokedBy`) freeing the handle while blocklisting the revoked material, and logs the revocation. Shares the registrar's process lock. |
| `RsaRevocationAuthority` | Implements `IRevocationAuthority`: verifies SHA512WITHRSA break-glass admin signatures over `RevocationPayload.Compose(target)` against the configured admin public key; fails closed when unconfigured. |
| `StaticAdminPublicKeySource` | Implements `IAdminPublicKeySource`: holds the per-deployment break-glass admin public RSA key (exported from the offline YubiKey / PIV applet), or null to leave revocation unconfigured. |
| `ProfileRepositoryServiceRegistration` | Static extension `AddEncryptedProfileRepository` that registers all encrypted profile storage dependencies into a `ServiceRegistry`, including the key-set registration policy (`IPublicKeySetRegistrar`, `IKeySetRotationVerifier`, `ISignatureProvider`) and the revocation policy (`IKeySetRevocation`, `IRevocationAuthority`, `IAdminPublicKeySource`). |
